...
def init_logger(filename):
    ...
    with open(filename, "w") as logfile:
        header = f"{'Time[ps]':>10} {'T[K]':>10}\n"
        logfile.write(header)
        print(header.strip())

def log_state(state, current_time_ps, filename):
    """Extracts and logs only Time and Temperature from the TorchSim state."""
    
    # --- 1. Temperature ---
    if callable(getattr(state, "calc_temperature", None)):
        T = float(state.calc_temperature().cpu()[0])
    else:
        T = float(state.calc_temperature.cpu()[0])
        
    # Format the log line
    log_line = f"{current_time_ps:10.4f} {T:10.1f}"
...
# FIXED: Replaced .to_atoms() with safe extraction loop
